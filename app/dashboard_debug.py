...
with tab3:
    ...
    st.subheader("Percentiles de la Plantilla")
    ...
    st.subheader("Ranking en la Plantilla")
...
with tab4:
    ...
    if last_7 > last_14:
        ...
        st.warning(
        "La tendencia de riesgo es ascendente."
    )

    else:

        st.success(
        "La tendencia de riesgo es estable o descendente."
    )
...
# =====================================================
    ...
    session_phase = selected_row["session_phase"]

    st.info(
    f"""
    Fecha: {selected_date}

    Fase del microciclo: {session_phase}

    Tipo de sesión: {selected_row['type_session']}
    """
)

# =====================================================
# COMPARATIVA VS PATRÓN DEL MICROCICLO
# =====================================================

    st.subheader(
    "Sesión vs Patrón del Microciclo"
)
...
# =====================================================
    ...
    if session_score < 80:
        ...
        st.success(
        "Sesión por debajo de la exigencia habitual de esta fase."
    )

    elif session_score < 120:

        st.info(
        "Sesión dentro de los parámetros habituales."
    )

    else:

        st.warning(
        "Sesión más exigente de lo habitual para esta fase."
    )

    st.divider()

# =====================================================
# RADAR VS PATRÓN ESPERADO
# =====================================================

    st.subheader(
    "Perfil de la Sesión vs Patrón Esperado"
)
...
# =====================================================
    ...
    if len(anomalies) > 0:
        ...
        for _, row in anomalies.iterrows():
            ...
            text.append(
            f"{row['Métrica']} ({row['Dif %']:+.1f}%)"
        )

        st.warning(
        "Desviaciones relevantes detectadas: "
        + ", ".join(text)
    )
        ...
        st.success(
        "La sesión se encuentra dentro del patrón habitual de esta fase del microciclo."
    )
